...
namespace QuodAstrum.NPCs
{
	...
	internal class CygmaH : Cygma
	{
		...
		public override void SetDefaults()
		{
			...
			npc.width = 175;
			npc.height = 175;
			//music = mod.GetSoundSlot(SoundType.Music, "Sounds/Music/DuneRaider");
			npc.boss = true;
	...
	internal class CygmaB : Cygma
	{
		...
		public override void CustomBehavior()
		{
			if (Main.netMode != NetmodeID.MultiplayerClient)
			{
				...
				if (attackCounter <= 0 && Vector2.Distance(npc.Center, target.Center) < 1000 && Collision.CanHit(npc.Center, 1, 1, target.Center, 1, 1))
				{
					Vector2 direction = (target.Center - npc.Center).SafeNormalize(Vector2.UnitX);
					direction = direction.RotatedByRandom(MathHelper.ToRadians(10));

					int projectile = Projectile.NewProjectile(npc.Center, direction * 5, ProjectileID.ShadowBeamHostile, 12, 5, Main.myPlayer);
		...
		public override void CustomBehavior()
		{
			if (Main.netMode != NetmodeID.MultiplayerClient)
			{
				...
				if (attackCounter <= 0 && Vector2.Distance(npc.Center, target.Center) < 1000 && Collision.CanHit(npc.Center, 1, 1, target.Center, 1, 1))
				{
					...
					int projectile = Projectile.NewProjectile(npc.Center, direction * 5, ProjectileID.CultistBossLightningOrb, 15, 5, Main.myPlayer);
					Main.projectile[projectile].timeLeft = 1200;
					attackCounter = 250;
					npc.netUpdate = true;
				}
	...
	public abstract class Cygma : StarWorm
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Cygma, Daughter of Andromeda");
		}
		...
		public override void Init()
		{
			minLength = 12;
			maxLength = 12;
			tailType = NPCType<CygmaT>();
			bodyType = NPCType<CygmaB>();
			headType = NPCType<CygmaH>();
			speed = 5.5f;
			turnSpeed = 0.1f;
		}
	}
	...
	public abstract class StarWorm : ModNPC
	{
		/* ai[0] = follower
		 * ai[1] = following
		...
		public static bool Line1 = false;
		public static bool Line2 = false;
		public static bool Line3 = false;
		public static bool Line4 = false;
		public static bool Line5 = false;
			...
			if (Main.netMode != NetmodeID.MultiplayerClient)
			{
				if (!tail && npc.ai[0] == 0f)
				{
					if (head)
					{
						...
						npc.ai[2] = (float)Main.rand.Next(minLength, maxLength + 6);
